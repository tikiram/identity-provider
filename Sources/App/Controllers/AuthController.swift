...
struct AuthControler: RouteCollection {
    ...
    func token(req: Request) async throws -> TokensResponse {
        ...
        if payload.grand_type == "password" {
            ...
            guard let email = payload.username, let password = payload.password else {
                throw Abort(.badRequest, reason: "email and password required")
            }
            
            let tokens = try await Auth(req).authenticate(email, password)
            
            return TokensResponse(access_token: tokens.accessToken, refresh_token: tokens.refreshToken, expires_in: ACCESS_TOKEN_EXPIRATION)
        }
        ...
        if payload.grand_type == "refresh_token" {
            ...
            let accessToken = try await Auth(req).getNewAccessToken(refreshToken: refreshToken)
            return TokensResponse(access_token: accessToken, refresh_token: nil, expires_in: ACCESS_TOKEN_EXPIRATION)
        }
